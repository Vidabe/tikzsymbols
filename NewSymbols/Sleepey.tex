    ...
    \begin{tikzpicture}
      [ x=1.92ex, y=1.92ex, line ~ width=0.096ex* #1 , scale=#1 ]
        ...
        \draw
          ...
          %% smaller
%          arc[start~angle=0,end~angle=130,x~radius=0.15,y~radius=0.24] ;
          arc[start~angle=0,end~angle=130,x~radius=0.15,y~radius=0.15] ;
        %% Pommel
        \def\xmid{0.55}
        \def\ymid{-0.15}
        \def\shift{1.2}
...
%  \def \three{\xmid - 0.555*\xr,  0.9*\yr}
%  \def \four{\xmid,0.9*\yr}
%  \def \five{ \xmid + 0.555*\xrII, 0.9*\yr}
%  \def \six{\xmid + 1*\xrII, 0.555*\yr}
%  \def \seven{\xmid + \xrII,0}
%
...
%%  \def \three{\xmid - 0.555*\xr,  0.9*\yr}
%%  \def \four{\xmid,1*\yr}
%%  \def \five{ \xmid + 0.6*\xr, 1*\yr}
%%  \def \six{\xmid + 1*\xr, 0.555*\yr}
%%  \def \seven{\xmid + \xr,0}
%%  \draw [red ] (\one) .. controls (\three) and (\five) .. (\seven)
%%          -- ++(0,-0.2)
%%          arc[start~angle=0,end~angle=130,x~radius=0.15,y~radius=0.24] 
%%          -- cycle    ;
%          
...
%      (\six) circle [radius=2pt]
%      (\seven) circle [radius=2pt];
%  \draw [rounded corners=1ex ] (\one) .. controls (\three) and (\five) .. (\seven);
%%                 .. controls (\five) and (\six) .. (\seven);
%    \end{tikzpicture}
